...
# Dark Terminal & Ultra-Sharp Vector Graphics CSS
st.markdown("""
<style>
    .stApp {
...
# Global Matplotlib Vector Format Config
...
def run_live_terminal():
    ...
    # Render Screen Output right below top controls
    st.markdown(f'<div class="terminal-screen">{clean_output}</div>', unsafe_allow_html=True)

    # Render Infinitely Sharp Vector SVG Charts
    figs = [plt.figure(i) for i in plt.get_fignums()]
    if figs:
        st.markdown("### 📊 Ultra-Sharp Vector SVG Charts")
        st.info("💡 Tip: Click the ⛶ Fullscreen icon in the top right corner of any chart to expand with zero pixelation!")
        for idx, fig in enumerate(figs, 1):
            ...
            # Export to SVG vector stream for infinite razor-sharp resolution
            svg_io = io.StringIO()
            fig.savefig(svg_io, format='svg', bbox_inches='tight', transparent=True)
            svg_data = svg_io.getvalue()
            
            with st.expander(f"🔍 Expand Razor-Sharp Chart #{idx}", expanded=True):
