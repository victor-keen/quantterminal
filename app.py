...
def _blocked_requests_post(url, *args, **kwargs):
    if isinstance(url, str) and "openrouter.ai" in url:
        class FakeResponse:
            status_code = 200
            def json(self):
                return {
                    "choices": [{
                        "message": {
                            "content": "[AI Bypassed] Displaying pure quantitative metrics."
                        }
                    }]
                }
            @property
            def text(self):
                return "[AI Bypassed] Displaying pure quantitative metrics."
        return FakeResponse()
...
def _bypassed_ai_call(self, *args, **kwargs):
    return ("[AI Bypassed] Displaying pure quantitative metrics.", 0.0)
...
for attr in dir(special):
    ...
    if isinstance(obj, type):
        ...
        if hasattr(obj, "run_ai_fundamental_analysis"):
            setattr(obj, "run_ai_fundamental_analysis", lambda self: "[AI Bypassed]")
        if hasattr(obj, "run_ai_trade_setup"):
            setattr(obj, "run_ai_trade_setup", lambda self: "[AI Bypassed]")

# ── Patch rich.prompt.Prompt.ask so it reads from our mocked builtins.input ──
try:
    from rich.prompt import Prompt as _RichPrompt
    _orig_prompt_ask = _RichPrompt.ask
