...
def capture_and_run_module(mod_id, tkr):
    ...
    # Mock terminal input to exit interactive sub-menus cleanly in web environment
    orig_input = builtins.input
    input_call_count = [0]
    def mocked_input(prompt=""):
        input_call_count[0] += 1
        if input_call_count[0] > 10:
            return "0"  # Force exit if stuck in endless menu loop
        return "0"      # Default back command for terminal sub-menus
...
if run_button or st.session_state.get("auto_run", False):
    ...
    capture_and_run_module(module_id, ticker)
else:
    st.markdown("""
    ...
    3. Click **🚀 Run Analysis Module** to view full outputs, metrics, and matplotlib graphs right inside your browser!
    """)
